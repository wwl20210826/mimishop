...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            ...
            <input type="button" value="查询" onclick="ajaxCondition()">
        </form>
    </div>
    <br>
    <div id="table">
                            ...
                            <nav aria-label="..." style="text-align:center;">
                                <ul class="pagination">
                                    <li>
                                        ...
                                        <a href="javascript:ajaxCondition(${info.prePage})" aria-label="Previous">
                                    ...
                                    <c:forEach begin="1" end="${info.pages}" var="i">
                                        <c:if test="${info.pageNum==i}">
                                            <li>
                                                ...
                                                <a href="javascript:ajaxCondition(${i})"
                                        ...
                                        <c:if test="${info.pageNum!=i}">
                                            <li>
                                                ...
                                                <a href="javascript:ajaxCondition(${i})">${i}</a>
                                            </li>
                                        </c:if>
                                    </c:forEach>
                                    <li>
                                        <%--  <a href="${pageContext.request.contextPath}/prod/split.action?page=1" aria-label="Next">--%>
                                        <a href="javascript:ajaxCondition(${info.nextPage})" aria-label="Next">
                                            <span aria-hidden="true">»</span></a>
                                    </li>
...
<script type="text/javascript">
    ...
    function deleteBatch() {
            ...
            if(zhi.length==0){
                ...
                //拼接ID
                    ...
                    var ispage = ${info.pageNum}
                    var pname = $("#pname").val();
                    var typeid = $("#typeid").val();
                    var lprice = $("#lprice").val();
                    var hprice = $("#hprice").val();
                    var str1 = "&pName="+pname+"&pType="+typeid+"&lPrice="+lprice+"&hPrice"+hprice+"&page="+ispage;
                    //alert(str+"11111111");
                    //发送请求到服务器端
                    window.location="${pageContext.request.contextPath}/prod/deleteBatch.action?str1="+str+str1;
    ...
    function del(pid) {
        if (confirm("确定删除吗")) {
            ...
            var lprice = $("#lprice").val();
            var hprice = $("#hprice").val();
            var str = "?pName="+pname+"&pType="+typeid+"&lPrice="+lprice+"&hPrice"+hprice+"&page="+ispage+"&pid="+pid;
    ...
    function one(pid, ispage) {
        ...
        var lprice = $("#lprice").val();
        var hprice = $("#hprice").val();
        var str = "?pName="+pname+"&pType="+typeid+"&lPrice="+lprice+"&hPrice"+hprice+"&pId="+pid+"&page="+ispage;
...
<script type="text/javascript">
    ...
    function ajaxCondition(page){
        ...
        $.ajax({
            ...
            data:{"pName":pname,"pType":typeid,"lPrice":lprice,"hPrice":hprice,"page":page},
            success:function(){
                $("#table").load("${pageContext.request.contextPath}/admin/product.jsp #table");
            }
        });
